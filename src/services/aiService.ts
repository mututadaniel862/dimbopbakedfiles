  ...
  try {
    ...
    if (lowerQuery.includes('blog') || lowerQuery.includes('article') || lowerQuery.includes('read')) {
      ...
      if (blogResults.length > 0) {
        const blogList = blogResults.slice(0, 5).map(b => 
          `📰 ${b.title}
📝 ${b.content ? b.content.substring(0, 150) : 'No preview available'}...`
        ).join('\n\n');
    ...
    if (searchResults.exactMatches.length > 0) {
      const productList = searchResults.exactMatches.slice(0, 5).map(p => 
        `🛍️ ${p.name} - $${Number(p.price)} ${getStockStatus(p.stock_quantity)}
    ...
    if (lowerQuery.includes('show') || lowerQuery.includes('all') || lowerQuery.includes('available') || lowerQuery.includes('products')) {
      ...
      if (availableProducts.length > 0) {
        const productList = availableProducts.map(p => 
          `🛍️ ${p.name} - $${Number(p.price)} (${p.stock_quantity} in stock) - ${p.categories?.name || 'General'}`
        ).join('\n');
 ...
 */
const smartBlogSearch = (blogs: any[], query: string) => {
  const lowerQuery = query.toLowerCase();
  ...
  return blogs.filter(blog => {
    ...
    return meaningfulTerms.some(term => 
      blogTitle.includes(term) || blogContent.includes(term)
    );
  });
};
