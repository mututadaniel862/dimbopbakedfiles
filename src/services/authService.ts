...
if (!ALLOWED_ROLES.includes(user.role || '')) {
  ...
  if (authProvider === 'email' || (!authProvider && password)) {
    ...
    if (!user.password_hash) {
      // Logic for converting social accounts or identifying social accounts
      if (user.auth_provider && user.auth_provider !== 'email') {
        throw new Error(`This account is linked to ${user.auth_provider}. Please log in with ${user.auth_provider}.`);
      }
...
export const getUser = async (identifier: {
  ...
  const orClauses = [
    identifier.username !== undefined ? { username: identifier.username } : null,
    identifier.email !== undefined ? { email: identifier.email } : null,
    identifier.phone !== undefined ? { phone: identifier.phone } : null
  ].filter((clause): clause is any => clause !== null);
