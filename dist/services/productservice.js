...
const updateCartItemQuantity = async (cartItemId, userId, newQuantity) => {
    // Delete the item if quantity is zero or less
    if (newQuantity <= 0) {
        return await (0, exports.deleteCartItem)(cartItemId, userId, true); // Restore stock when deleting
    }
    // Get cart item with product info
    const cartItem = await prisma.cart.findFirst({
        where: {
...
//   });
// };
const deleteCartItem = async (cartItemId, userId, restoreStock = true // Add parameter to control stock restoration
) => {
    const cartItem = await prisma.cart.findFirst({
        where: {
            id: cartItemId,
