...
const getFallbackResponseWithDB = async (query) => {
    ...
    try {
        ...
        if (lowerQuery.includes('product') || lowerQuery.includes('phone') || lowerQuery.includes('price')) {
            ...
            const matchingProducts = products.filter(p => p.name.toLowerCase().includes(lowerQuery) ||
                p.description?.toLowerCase().includes(lowerQuery) ||
                p.categories?.name.toLowerCase().includes(lowerQuery));
            if (matchingProducts.length > 0) {
                const productList = matchingProducts.slice(0, 3).map(p => `📱 ${p.name} - $${Number(p.price)} (${p.stock_quantity && p.stock_quantity > 0 ? `${p.stock_quantity} in stock` : 'Out of stock'})`).join('\n');
